...
public class MahoLuban {
	...
	private static final String TAG = "Luban";//保留鲁班的TAG。
	private static String DEFAULT_DISK_CACHE_DIR = "maho";
	...
	public MahoLuban setCacheName (String cacheName){
		//会执行两次，感觉异常难受
		mCacheDir = getPhotoCacheDir(null,cacheName);
		return this;
	}
	...
	private static File getPhotoCacheDir(Context context, String cacheName) {
		File cacheDir;
		if(cacheName.equals("")) {
			cacheDir = context.getCacheDir();//私有目录
		}else {
			cacheDir = new File("/sdcard/" + cacheName);//自定义目录
		}if (cacheDir != null) {
	...
	public void start() {
		...
		try {
			for (File file : mFileList) {
				...
				mOnMahoCompressLister.onFinishOne(file);
			}
